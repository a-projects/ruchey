...
* [Переключатель раскладок по CapsLock](https://github.com/a-projects/ruchey/blob/main/RucheySwitcher/README.md)
...
* Часть спецсимволов вынесены на третий уровень, т.е. набираются при зажатом AltGr (правый Alt)
...
# Рекомендации
При наборе фигурных и квадратных скобок, а также символов меньше и больше методом "слепой печати", рекомендуется переносить кисть так, чтобы маркером являлась не клавиша "О" ("J"), а правый Alt (AltGr). Т.е. кисть смещается, как если бы была необходимость воспользоваться стрелками.

# Участие в разработке
Свои вопросы, замечания просто пишите мне на почту <baryshkin_aa :dog: mail.ru>. Я постараюсь на них ответить, пояснить некоторый ход мыслей и решений. Конкретные ошибки закидывайте в issue.
...
**Вопрос:** В «Ручей» решены проблемы, присущие русской раскладке (ЙЦУКЕН)?
**Ответ:** Нет, задача не стоит исправить этот мир, а только дать равные возможности.

**Вопрос:** Снижается ли скорость набора технических текстов?
**Ответ:** Для русского варианта не снижается, наоборот увеличивается, ведь нет необходимости переключения. Для английского, снижается пока не привыкните, но лично мне переход не доставил неудобств.

**Вопрос:** Я набираю методом «слепой печати» как это отразится на навыках?
...
**Вопрос:** Что изменилось с версии v2.0?
**Ответ:** Английский и русский варианты теперь полностью базируются на русской раскладке.
